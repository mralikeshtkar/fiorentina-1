<?php

return [
    'login' => [
        'username' => 'Email/Nome utente',
        'email' => 'Email',
        'password' => 'Password',
        'title' => 'Accesso utente',
        'remember' => 'Ricordami?',
        'login' => 'Accedi',
        'placeholder' => [
            'username' => 'Inserisci il tuo nome utente o indirizzo email',
            'email' => 'Inserisci il tuo indirizzo email',
            'password' => 'Inserisci la tua password',
        ],
        'success' => 'Accesso effettuato con successo!',
        'fail' => 'Nome utente o password errati.',
        'not_active' => 'Il tuo account non è ancora stato attivato!',
        'banned' => 'Questo account è stato bannato.',
        'logout_success' => 'Disconnessione avvenuta con successo!',
        'dont_have_account' => 'Non hai un account su questo sistema, per favore contatta l\'amministratore per maggiori informazioni!',
    ],
    'forgot_password' => [
        'title' => 'Password dimenticata',
        'message' => '<p>Hai dimenticato la password?</p><p>Per favore inserisci il tuo indirizzo email. Il sistema invierà un email con un link attivo per reimpostare la tua password.</p>',
        'submit' => 'Invia',
    ],
    'reset' => [
        'new_password' => 'Nuova password',
        'password_confirmation' => 'Conferma nuova password',
        'email' => 'Email',
        'title' => 'Reimposta la tua password',
        'update' => 'Aggiorna',
        'wrong_token' => 'Questo link è invalido o scaduto. Per favore prova ad usare nuovamente il modulo di reimpostazione.',
        'user_not_found' => 'Questo nome utente non esiste.',
        'success' => 'Password reimpostata con successo!',
        'fail' => 'Il token è invalido, il link per reimpostare la password è scaduto!',
        'reset' => [
            'title' => 'Email per reimpostare la password',
        ],
        'send' => [
            'success' => 'Una email è stata inviata al tuo account email. Per favore controlla e completa questa azione.',
            'fail' => 'Impossibile inviare l\'email in questo momento. Per favore riprova più tardi.',
        ],
        'new-password' => 'Nuova password',
        'placeholder' => [
    'new_password' => 'Inserisci la tua nuova password',
    'new_password_confirmation' => 'Conferma la tua nuova password',
],
    ],
    'email' => [
    'reminder' => [
        'title' => 'Email per reimpostare la password',
    ],
],
    'password_confirmation' => 'Conferma password',
    'failed' => 'Fallito',
    'throttle' => 'Rallenta',
    'not_member' => 'Non sei ancora un membro?',
    'register_now' => 'Registrati ora',
    'lost_your_password' => 'Hai perso la password?',
    'login_title' => 'Admin',
    'login_via_social' => 'Accedi con i social network',
    'back_to_login' => 'Torna alla pagina di accesso',
    'sign_in_below' => 'Accedi qui sotto',
    'languages' => 'Lingue',
    'reset_password' => 'Reimposta Password',
    'settings' => [
    'email' => [
        'title' => 'ACL',
        'description' => 'Configurazione email ACL',
    ],
],
];
